...
export async function GET() {
  try {
    const products = await prisma.product.findMany({
      where: {
        ...
        OR: [
          ...
          { category: { name: { contains: 'cable', mode: 'insensitive' } } },
          { category: { name: { contains: 'redes', mode: 'insensitive' } } },
        ]
      },
    ...
    // Combine database cable products with curated fallbacks (avoiding duplicates by id/name)
    ...
    console.error("Error in /api/web/bobinas-cables route:", error);
