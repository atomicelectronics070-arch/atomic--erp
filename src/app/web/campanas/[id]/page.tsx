      ...
      <div className="max-w-7xl mx-auto px-6 mt-10 grid lg:grid-cols-2 gap-16">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-gradient-to-r from-slate-800 to-slate-900 rounded-2xl p-5 flex items-center gap-4 shadow-xl">
            ...
            <div>
              ...
              <p className="text-slate-400 text-xs mt-0.5">{specsObj.garantia || '1 año en piezas'} · Marca {specsObj.marca || 'ATOMIC INDUSTRIAL'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-8">
          ...
          <div>
            ...
            {/* Precio de venta ÚNICO (sin precio de costo tachado) */}
          ...
          {hasSpecs && (
            <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
              <h2 className="text-xl font-bold text-slate-900 border-b border-slate-100 pb-3 flex items-center gap-2">
                <span>📋</span> Ficha Técnica Oficial
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                {Object.entries(specsObj).map(([key, val]) => (
                  <div key={key} className="bg-slate-50 p-3 rounded-xl border border-slate-100 flex flex-col">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1">
                      {SPEC_ICONS[key] || '▪️'} {SPEC_LABELS[key] || key}
                    </span>
                    <span className="font-semibold text-slate-800 mt-1 text-sm">{val}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
            <h2 className="text-xl font-bold text-slate-900 border-b border-slate-100 pb-3 flex items-center gap-2">
              <span>📖</span> Descripción & Detalles del Producto
            </h2>
