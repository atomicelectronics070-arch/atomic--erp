    ...
    return (
        <div className="min-h-screen bg-[#F4F1EB] text-[#0F1923] flex flex-col lg:flex-row overflow-hidden h-screen relative">
                ...
                {showAiTutor && (
                    ...
                    >
                        ...
                        <div className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar-hidden">
                            <div className="bg-slate-900/50 backdrop-blur-xl border-slate-700/50/5 p-6 border-l-2 border-[#E8341A]">
                            ...
                            <div className="space-y-6 opacity-40">
                                ...
                                <div className="flex justify-start">
                                    <div className="bg-slate-900/50 backdrop-blur-xl border-slate-700/50/5 border border-white/10 p-4 text-[10px] text-white/50 uppercase tracking-widest max-w-[80%] italic">
                        ...
                        <div className="p-10 border-t border-white/5 bg-black/20">
                            <div className="relative group">
                                <input 
                                    ...
                                    className="w-full bg-slate-900/50 backdrop-blur-xl border-slate-700/50/5 border border-white/10 p-5 text-white text-[10px] font-black uppercase tracking-[0.2em] outline-none focus:border-[#E8341A] transition-all pr-16"
                                />
            ...
            <aside className="w-full lg:w-[400px] bg-[#0F1923] text-white flex flex-col h-auto lg:h-full relative z-30 shadow-2xl">
                <div className="p-8 border-b border-white/5 bg-slate-900/50 backdrop-blur-xl border-slate-700/50/[0.02]">
                    ...
                    {!isPublic && course.enrollments?.[0] && (
                        <div className="mt-10 space-y-4 bg-slate-900/50 backdrop-blur-xl border-slate-700/50/5 p-6 rounded-none border border-white/5">
                            ...
                            <div className="w-full h-1.5 bg-slate-900/50 backdrop-blur-xl border-slate-700/50/5 rounded-none overflow-hidden">
                    ...
                    {isPublic && (
                        <div className="mt-10 p-6 border border-blue-500/30 bg-blue-500/5">
                            ...
                            <Link href="/login" className="block w-full py-3 bg-gradient-to-r from-cyan-500 to-indigo-600 shadow-[0_0_15px_rgba(34,211,238,0.3)] hover:scale-105 transition-all text-white text-center text-[9px] font-black uppercase tracking-[0.3em] hover:bg-blue-500 transition-all">
                                INICIAR SESIÓN
                            </Link>
                        </div>
                    )}
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar-hidden bg-slate-900/50 backdrop-blur-xl border-slate-700/50/[0.01]">
                    {course.lessons.map((l: any, index: number) => {
                        ...
                        return (
                            <Link 
                                ...
                                key={l.id}
                                className={`flex items-start gap-6 p-8 border-b border-white/5 transition-all relative group ${isActive ? 'bg-slate-900/50 backdrop-blur-xl border-slate-700/50/5' : 'hover:bg-slate-900/50 backdrop-blur-xl border-slate-700/50/[0.03]'}`}
                            >
            ...
            <main className="flex-1 flex flex-col h-full relative bg-[#F4F1EB] overflow-hidden">
                ...
                >
                    <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-xl border-slate-700/50/20 -translate-y-full group-hover:translate-y-0 transition-transform duration-500"></div>
                        ...
                        {quizStep === "QUIZ" ? (
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#0F1923] p-16 text-white border-t-8 border-[#E8341A] shadow-2xl">
                                ...
                                <div className="space-y-4">
                                    {quiz[currentQuestion].a.map((option: string, i: number) => (
                                        <button 
                                            ...
                                            className={`w-full p-6 text-left text-[11px] font-black uppercase tracking-widest border transition-all ${selectedAnswer === i ? 'bg-[#E8341A] border-[#E8341A] text-white' : 'bg-slate-900/50 backdrop-blur-xl border-slate-700/50/5 border-white/10 text-white/50 hover:bg-slate-900/50 backdrop-blur-xl border-slate-700/50/10'}`}
                                        >
                                ...
                                <button 
                                    ...
                                    className="w-full py-6 bg-slate-900/50 backdrop-blur-xl border-slate-700/50 text-[#0F1923] mt-12 font-black text-[12px] uppercase tracking-[0.6em] hover:bg-[#E8341A] hover:text-white transition-all disabled:opacity-20"
                                >
                                    Siguiente Fase
                                </button>
                            </motion.div>
                            ...
                            <>
                                ...
                                <div className="mt-32 pt-12 border-t border-black/5 flex flex-col sm:flex-row items-center justify-between gap-10">
                                    ...
                                    {(quiz.length > 0 && quizStep === "CONTENT" && !isCompleted && !isPublic) ? (
                                        ...
                                        isPublic ? (
                                            <Link href="/login" className="px-12 py-5 bg-[#0F1923] text-white text-[11px] font-black uppercase tracking-[0.4em] hover:bg-gradient-to-r from-cyan-500 to-indigo-600 shadow-[0_0_15px_rgba(34,211,238,0.3)] hover:scale-105 transition-all transition-all flex items-center gap-4 skew-x-[-12deg]">
