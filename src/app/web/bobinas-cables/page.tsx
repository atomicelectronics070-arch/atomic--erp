...
import { Search, ChevronLeft, ChevronRight, Zap, Shield, Sparkles, Check, MessageCircle, Layers, Cpu, ShieldCheck, CheckCircle2, Award } from "lucide-react"
...
export default function BobinasCablesLandingPage() {
        ...
        const init = async () => {
            ...
            try {
                const [pRes, sRes] = await Promise.all([
                    fetch("/api/web/bobinas-cables").then(r => r.json()),
    ...
    if (loading) return (
        <div className="min-h-screen bg-[#05070c] text-white flex items-center justify-center">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-xs font-mono font-black text-cyan-300 uppercase tracking-[0.3em]">Cargando Catálogo de Bobinas de Cable...</p>
            </div>
        </div>
    )
    ...
    return (
        <div className="min-h-screen bg-[#05070c] text-slate-100 font-sans pb-32">
            ...
            <header className="max-w-7xl mx-auto px-4 md:px-6 pt-8 pb-8 space-y-6">
                ...
                {/* 📊 TECHNICAL COMPARISON CARDS (COBRE PURO VS CCA VS BLINDADO) */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
                    
                    {/* CARD 1: 100% COBRE PURO */}
            ...
            <main className="max-w-7xl mx-auto px-4 md:px-6 py-4">
                ...
                {filtered.length === 0 ? (
                    <div className="py-20 text-center border border-dashed border-[#1e293b] rounded-2xl bg-[#0e1424] p-6 space-y-3">
                        <Zap className="w-12 h-12 text-slate-500 mx-auto" strokeWidth={1.5} />
                        <h3 className="text-sm font-bold text-slate-300 uppercase font-mono">No se encontraron bobinas con ese filtro</h3>
                        <p className="text-xs text-slate-400 max-w-md mx-auto">
                            ¿Buscas un metraje especial o especificación técnica particular? Escríbenos por WhatsApp para cotizártelo directamente con precio de importación.
                        </p>
                        <button
                            onClick={() => { setSearch(""); setSelectedTypeFilter("TODOS"); }}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {filtered.map((p, i) => {
                            ...
                            return (
                                ...
                                >
                                    <div>
                                        {/* PURE WHITE CONTAINER FOR PRODUCT IMAGE WITH ULTRA-THIN GOLD BORDER & ELECTRIC BLUE SHADOW */}
                                        <div 
                                            style={{
                                                backgroundColor: '#ffffff',
                                                border: '2px solid #d4af37',
