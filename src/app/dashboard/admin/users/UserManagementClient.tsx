...
export default function UserManagementClient({ users: initialUsers }: Props) {
    ...
    const renderTabButton = (id: string, label: string, count: number) => (
        <button
            ...
            className={`px-6 py-4 text-xs font-bold uppercase tracking-widest transition-all border-b-2 flex items-center space-x-2 ${activeTab === id
        ...
        >
            <span>{label}</span>
            <span className={`px-2 py-0.5 rounded-none text-[10px] ${activeTab === id ? 'bg-orange-600 text-white' : 'bg-neutral-100  text-neutral-500'}`}>{count}</span>
        </button>
    )
                ...
                <div className="p-3 flex items-center">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="pl-9 pr-4 py-2 bg-neutral-50  border border-neutral-100  rounded-none text-xs outline-none focus:ring-2 focus:ring-orange-500 w-full md:w-64"
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="divide-y divide-neutral-50 ">
                {usersToShow.length === 0 ? (
                    ...
                    usersToShow.map(user => (
                        <div key={user.id} className="p-8 transition-colors hover:bg-neutral-50/50 :bg-neutral-800/20 group">
                            <div className="flex flex-col lg:flex-row gap-8">

                                {/* Left Col: Identity */}
                                <div className="flex-1 flex items-start space-x-6">
                                    <div className="w-16 h-16 rounded-none bg-orange-600 flex items-center justify-center text-white font-bold text-2xl shadow-lg shadow-orange-500/20 shrink-0">
                                        {user.name?.[0] || "?"}
                                    </div>
                                    <div>
                                        <h3 className="text-xl font-bold text-neutral-900  tracking-tight flex items-center">
                                            {user.name}
                                            {user.status === "PENDING" && <span className="ml-3 text-[9px] font-bold px-2 py-0.5 bg-orange-600 text-white rounded-none uppercase tracking-widest">Nuevo</span>}
                                        </h3>
                                        <div className="flex items-center space-x-4 mt-1 text-[11px] font-bold text-neutral-400 uppercase tracking-widest">
                                            <span className="flex items-center"><Mail size={14} className="mr-1.5 text-orange-500" /> {user.email}</span>
                                            <span className="flex items-center"><Clock size={14} className="mr-1.5 text-orange-500" /> {new Date(user.createdAt).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="lg:w-72 flex flex-col justify-between border-t lg:border-t-0 lg:border-l border-neutral-50  pt-8 lg:pt-0 lg:pl-8">
                                        ...
                                        {user.status === "PENDING" && (
                                            <div className="flex flex-col space-y-2">
                                                <button
                                                    ...
                                                    className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-4 rounded-none flex items-center justify-center transition-all disabled:opacity-50 text-[10px] uppercase tracking-widest shadow-lg shadow-orange-500/20"
                                                >
                                        ...
                                        {user.status === "APPROVED" && (
                                            <div className="space-y-4">
                                                <div className="relative">
                                                    ...
                                                    <div className="relative">
                                                        <Shield className="absolute left-4 top-1/2 -translate-y-1/2 text-orange-500" size={16} />
                                                        <select
                                                            value={user.role}
                                                            onChange={(e) => handleRoleChange(user.id, e.target.value)}
                                                            className="w-full bg-neutral-50  border border-neutral-100  text-neutral-900  text-xs font-bold rounded-none pl-11 pr-4 py-3.5 outline-none focus:ring-2 focus:ring-orange-500 appearance-none transition-all"
                                                        >
                                    ...
                                    <div className="mt-8 pt-6 border-t border-neutral-50 flex justify-between items-center">
                                        ...
                                        <div className="flex items-center space-x-2 text-[10px] font-bold uppercase tracking-widest">
                                            ...
                                            <span className={user.status === 'APPROVED' ? 'text-green-500' : user.status === 'REJECTED' ? 'text-red-500' : 'text-orange-600'}>
                                                {user.status}
                                            </span>
                                        </div>
                                    </div>
