...
import {
    BookOpen, PlayCircle, Users, Search, GraduationCap,
    Plus, Edit3, Trash2, Globe, EyeOff, X,
...
export default function DashboardAcademyClient() {
        ...
        <div className="space-y-8 pb-20 animate-in fade-in duration-500">
            ...
            <div className="flex items-center gap-2 p-1.5 bg-slate-900/90 border border-slate-800 rounded-2xl w-fit">
                <Link
                    ...
                    className="px-5 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center gap-2 bg-gradient-to-r from-indigo-500 to-cyan-500 text-white shadow-lg"
                >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-900/80 border border-slate-800 rounded-3xl p-6 shadow-2xl backdrop-blur-xl">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-indigo-950/40 border border-indigo-500/40 rounded-2xl flex items-center justify-center">
                        <GraduationCap size={24} className="text-indigo-400" />
                    </div>
                    <div>
                        <h1 className="text-2xl font-black text-white uppercase tracking-tight">Academia Atomic</h1>
                        <p className="text-xs text-slate-400 font-mono mt-0.5 uppercase tracking-widest">Panel de Gestión de Contenido Educativo & Cursos</p>
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <button onClick={loadData} className="p-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-400 hover:text-white hover:bg-slate-800 transition-all cursor-pointer">
                        <RefreshCw size={16} />
                    </button>
                    {isAdmin && (
                        <button
                            onClick={() => setShowCreate(true)}
                            className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-cyan-600 text-white rounded-xl font-bold text-xs hover:from-indigo-500 hover:to-cyan-500 transition-all shadow-lg shadow-indigo-500/20 cursor-pointer"
                        >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                {stats.map((s, i) => (
                    <div key={i} className={`bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-lg flex items-center gap-4`}>
                        <div className={`w-10 h-10 ${s.bg} rounded-xl flex items-center justify-center flex-shrink-0`}>
                            <s.icon size={18} className={s.color} />
                        </div>
                        <div>
                            <p className="text-2xl font-black text-white">{s.value}</p>
                            <p className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-wider">{s.label}</p>
                        </div>
                    </div>
                ))}
            </div>
