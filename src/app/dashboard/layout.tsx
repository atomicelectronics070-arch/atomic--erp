...
export default function DashboardLayout({
    ...
    if (status === "loading" || !session) {
        return (
            <div className="h-screen w-screen flex flex-col items-center justify-center bg-[#020617] gap-8">
                ...
                <div className="font-black text-secondary uppercase tracking-[0.8em] animate-pulse text-xs italic text-center px-8">Sincronizando Nodo Central...</div>
            </div>
        )
    }
    ...
    return (
        <div className="flex h-screen bg-[#020617] text-white overflow-hidden font-sans relative selection:bg-secondary/30 selection:text-white">
            {/* Background Orbs - Deep Visual Hierarchy */}
            <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
                <div className="absolute top-[-10%] right-[-5%] w-[50%] h-[50%] rounded-full bg-azure-500/5 blur-[120px] animate-pulse transition-all duration-10000" />
                <div className="absolute bottom-[-10%] left-[-5%] w-[45%] h-[45%] rounded-full bg-tomato-500/5 blur-[120px] animate-pulse transition-all duration-7000" />
                <div className="absolute top-[30%] left-[20%] w-[30%] h-[30%] rounded-full bg-purple-500/5 blur-[100px]" />
                <div className="absolute inset-0 bg-[url('/grid.svg')] opacity-[0.03] bg-fixed"></div>
            </div>

            {/* Top Security Line (Visual Accent) */}
            <div className="fixed top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-secondary/50 to-transparent z-[100]"></div>
            ...
            `}>
                <div className="h-40 flex flex-col items-center justify-center px-10 border-b border-white/5 relative group bg-white/[0.02]">
                    ...
                    <button
                        onClick={() => setSidebarOpen(false)}
                        className="absolute top-8 right-8 lg:hidden text-slate-500 hover:text-secondary transition-all hover:rotate-90"
                    >
                        <X size={24} />
                    </button>
                    <div className="flex flex-col items-center relative z-10 transition-transform duration-700 group-hover:scale-105">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="w-1.5 h-1.5 rounded-full bg-secondary shadow-[0_0_8px_rgba(255,99,71,0.5)] animate-pulse"></div>
                    ...
                    <div className="space-y-10">
                        ...
                        {(role === "ADMIN" || role === "MANAGEMENT" || role === "SALESPERSON") && (
                            ...
                            >
                                ...
                                >
                                    <Mail size={18} className="text-slate-600 group-hover:text-secondary transition-colors" />
                                    <span className="tracking-[0.2em] uppercase">Mensajería_Env</span>
                                    {unreadCount > 0 && (
                                        <span className="absolute right-4 top-1/2 -translate-y-1/2 bg-secondary text-white text-[9px] font-black px-2.5 py-0.5 rounded-full shadow-[0_0_15px_rgba(255,99,71,0.5)] animate-pulse">
                                            {unreadCount}
                                        </span>
                                    )}
                                </Link>
                                ...
                                <Link
                                    href="/web"
                                    target="_blank"
                                    className="flex items-center space-x-4 px-5 py-4 rounded-2xl text-[10px] font-black text-secondary hover:bg-secondary/10 transition-all group border border-secondary/20 mt-6 bg-secondary/5 italic shadow-2xl shadow-secondary/5"
                                >
                                    <ExternalLink size={18} className="text-secondary group-hover:rotate-12 transition-transform" />
                ...
                <div className="p-10 shrink-0 border-t border-white/5 bg-white/[0.02]">
                    <div className="flex items-center space-x-6 mb-10 px-2 transition-all group cursor-default">
                        <div className="w-14 h-14 rounded-2xl glass-panel !bg-slate-900 border-white/10 shadow-2xl flex items-center justify-center font-black text-lg group-hover:scale-110 transition-transform duration-500 shadow-secondary/10 text-white italic">
                        ...
                        <div className="flex-1 overflow-hidden">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <p className="text-[9px] font-black text-secondary uppercase tracking-[0.3em] truncate italic opacity-80">
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden relative z-10 transition-all duration-700">
                ...
                <header className="h-28 glass-panel !bg-slate-950/40 !border-b !border-white/5 flex items-center justify-between px-10 lg:px-16 shrink-0 z-50 backdrop-blur-3xl shadow-2xl">
                    <div className="flex items-center space-x-10">
                        ...
                        <div className="hidden sm:flex flex-col">
                            <div className="flex items-center gap-4 mb-1">
                                <div className="w-2.5 h-2.5 rounded-full bg-secondary shadow-[0_0_12px_rgba(255,99,71,0.8)] animate-pulse"></div>
                    ...
                    <div className="flex items-center space-x-10">
                        ...
                        {(role === "ADMIN" || role === "MANAGEMENT") && (
                            <Link href="/dashboard/admin/settings" className="p-4 glass-panel text-slate-500 hover:text-secondary transition-all rounded-2xl shadow-xl hover:scale-105 active:scale-95 border-white/10 group">
...
function NavLink({ href, icon, label, isActive }: { href: string; icon: React.ReactNode; label: string; isActive?: boolean }) {
    return (
        <Link
            ...
            `}
        >
            <div className={`absolute top-0 left-0 w-1 h-full bg-secondary transition-transform duration-700 ${isActive ? 'scale-y-100' : 'scale-y-0'}`}></div>
            <span className={`transition-all duration-500 group-hover:scale-110 ${isActive ? 'text-secondary scale-110' : 'group-hover:text-primary'}`}>
                {icon}
            </span>
...
function CollapsibleSection({ label, children, isOpen, onToggle }: { label: string; children: React.ReactNode; isOpen: boolean; onToggle: () => void }) {
    return (
        <div className="space-y-4">
            ...
            >
                <div className="flex items-center gap-4">
                    <div className={`w-1 h-4 bg-slate-800 transition-all duration-500 group-hover:bg-slate-400 ${isOpen ? 'bg-secondary h-6' : ''}`}></div>
                ...
                {isOpen ? (
                    <ChevronDown size={14} className="text-secondary animate-bounce-subtle" />
                ) : (
