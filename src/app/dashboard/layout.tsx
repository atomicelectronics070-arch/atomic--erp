                        ...
                        {(role === "ADMIN" || role === "MANAGEMENT" || role === "SALESPERSON") && (
                            ...
                            >
                                <Link
                                    ...
                                    className="flex items-center space-x-4 px-5 py-4 rounded-none text-[10px] font-black text-slate-500 hover:bg-white/5 hover:text-white transition-all group relative border border-transparent hover:border-white/5 italic"
                                >
                                    ...
                                    {unreadCount > 0 && (
                                        <span className="absolute right-4 top-1/2 -translate-y-1/2 bg-primary text-white text-[9px] font-black px-2.5 py-0.5 rounded-none shadow-[0_0_15px_rgba(99,102,241,0.5)] animate-pulse">
                                            {unreadCount}
                                        </span>
                                    )}
                                </Link>
                                ...
                                <Link
                                    href="/web"
                                    target="_blank"
                                    className="flex items-center space-x-4 px-5 py-4 rounded-none text-[10px] font-black text-primary hover:bg-primary/10 transition-all group border border-primary/20 mt-6 bg-primary/5 italic shadow-2xl shadow-primary/5"
                                >
                ...
                <header className="h-28 glass-panel !bg-slate-950/40 !border-b !border-white/5 flex items-center justify-between px-10 lg:px-16 shrink-0 z-50 backdrop-blur-3xl shadow-2xl">
                    <div className="flex items-center space-x-10">
                        <button
                            ...
                            className="lg:hidden p-4 glass-panel text-slate-500 hover:text-white transition-all rounded-none shadow-xl hover:scale-105 active:scale-95 border-white/10"
                        >
                            <Menu size={28} />
                        </button>
...
function NavLink({ href, icon, label, isActive }: { href: string; icon: React.ReactNode; label: string; isActive?: boolean }) {
    return (
        <Link
            href={href}
            className={`
                flex items-center space-x-5 px-6 py-4.5 rounded-none-[1.5rem] transition-all duration-500 group relative overflow-hidden
                ${isActive 
