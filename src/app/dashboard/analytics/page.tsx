        ...
        <div className="space-y-16 pb-32 animate-in fade-in duration-1000 relative z-10">
            ...
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-12 border-b border-white/5 pb-20 relative z-10">
                ...
                <div className="flex items-center gap-10">
                    <button 
                        onClick={loadStats} 
                        className="p-8 glass-panel hover:bg-white/10 text-slate-500 hover:text-secondary transition-all rounded-[2rem] group border-white/5 shadow-2xl active:scale-95"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 relative z-10">
                ...
                <button 
                    ...
                    className="glass-panel !bg-slate-950/80 p-12 text-white shadow-[0_50px_100px_-20px_rgba(0,0,0,0.8)] relative overflow-hidden text-left hover:scale-[1.03] transition-all border-b-8 border-secondary/40 rounded-[3rem] group ring-1 ring-white/10 backdrop-blur-3xl"
                >
            ...
            <div className="glass-panel border-white/10 shadow-[0_60px_120px_-30px_rgba(0,0,0,0.9)] overflow-hidden rounded-[4rem] relative z-10 backdrop-blur-[40px] border-t border-l border-white/5 ring-1 ring-white/5 group">
                <div className="p-16 border-b border-white/10 flex flex-col xl:flex-row justify-between items-center gap-14 bg-white/[0.03]">
                    ...
                    <div className="flex glass-panel !bg-slate-950/80 p-3 rounded-[2rem] border-white/10 ring-1 ring-white/5 shadow-inner backdrop-blur-2xl">
                ...
                {showQuotesOverlay && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-end overflow-hidden p-0 md:p-8">
                        ...
                        <motion.div 
                            ...
                            className="w-full md:w-[900px] h-full glass-panel !bg-slate-950/70 !border-l !border-white/15 relative flex flex-col shadow-[0_0_200px_rgba(0,0,0,1)] md:rounded-l-[5rem] overflow-hidden backdrop-blur-[100px]"
                        >
                            <div className="p-16 border-b border-white/10 flex justify-between items-center bg-white/[0.02] shrink-0">
                                ...
                                <button 
                                    onClick={() => setShowQuotesOverlay(false)} 
                                    className="p-6 bg-slate-900 border border-white/15 text-slate-500 hover:text-white hover:rotate-90 transition-all duration-700 rounded-[1.5rem] shadow-2xl group active:scale-90"
                                >
                                    <X size={40} />
                                </button>
                            </div>
                                ...
                                {isAdmin ? (
                                    data.quoteData.map((user: any) => (
                                        <div key={user.id} className="space-y-10 animate-in slide-in-from-bottom-20 duration-1000">
                                            <div className="flex items-center gap-8 glass-panel !bg-slate-900/40 border-white/15 p-8 rounded-[3rem] shadow-2xl border-l-[12px] border-l-secondary relative overflow-hidden group hover:!bg-slate-900/60 transition-all">
                                                ...
                                                <div className="w-20 h-20 glass-panel !bg-slate-950 text-white flex items-center justify-center text-3xl font-black shadow-2xl rounded-[1.5rem] italic group-hover:rotate-6 transition-all border-white/10">
                                                    {user.name[0]}
                                                </div>
                                                <div className="space-y-2">
                                                    <h3 className="text-2xl font-black text-white uppercase tracking-tighter italic leading-none">{user.name}</h3>
                                                    <span className="bg-white/5 text-slate-500 text-[10px] px-6 py-2 font-black uppercase tracking-[0.5em] border border-white/10 rounded-full mt-4 inline-block italic">
                                            ...
                                            <div className="grid grid-cols-1 gap-8 ps-12 border-l-4 border-white/5">
                                                ...
                                                {user.latest.length === 0 && (
                                                    <div className="p-12 border border-dashed border-white/15 rounded-[2.5rem] text-[11px] text-slate-700 font-black uppercase tracking-[0.6em] italic flex items-center justify-center gap-6 opacity-40">
    ...
    return (
        <div className="glass-panel p-12 relative overflow-hidden group hover:scale-[1.03] transition-all duration-700 rounded-[3rem] border-white/5 shadow-2xl backdrop-blur-3xl ring-1 ring-white/5">
            ...
            <div className="flex justify-between items-start mb-14">
                <div className={`p-6 bg-slate-900/80 text-white shadow-2xl border border-white/10 rounded-2xl ring-1 ring-white/10 group-hover:rotate-12 transition-transform duration-500`}>
                    {icon}
                </div>
                <span className={`text-[10px] font-black text-${accentColor} bg-${accentColor}/10 px-6 py-2.5 uppercase tracking-widest rounded-2xl border border-${accentColor}/20 italic shadow-2xl shadow-${accentColor}/10`}>
                    {meta}
                </span>
            </div>
            ...
            <div className="mt-12 h-3 w-full bg-slate-900 rounded-full overflow-hidden border border-white/10 shadow-inner p-[3px] relative">
                <div className={`h-full bg-gradient-to-r ${barGradient} rounded-full transition-all duration-2000 ease-out`} style={{ width: `${progress}%`, boxShadow: `0 0 20px ${shadowColor}` }}></div>
            </div>
...
function QuoteRow({ quote }: { quote: any }) {
    return (
        <div className="flex items-center justify-between p-10 glass-panel !bg-slate-900/30 border-white/10 hover:!bg-white/[0.08] hover:border-secondary/50 transition-all group rounded-[2.5rem] shadow-2xl relative overflow-hidden border-t border-l border-white/5 backdrop-blur-xl">
            ...
            <div className="flex items-center gap-12">
                <div className="p-6 bg-slate-950/80 border border-white/15 text-slate-700 group-hover:text-secondary group-hover:rotate-12 transition-all rounded-2xl shadow-inner group-hover:scale-110">
                    <FileText size={32} />
                </div>
                <div>
                    ...
                    <div className="flex items-center gap-5 mt-3">
                        <div className="w-2 h-2 rounded-full bg-azure-500 shadow-[0_0_8px_rgba(45,212,191,0.6)]" />
