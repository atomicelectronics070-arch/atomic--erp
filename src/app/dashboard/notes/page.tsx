    ...
    return (
        <div className="space-y-12 pb-32 animate-in fade-in duration-1000 relative">
            ...
            <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
                <div className="absolute top-[5%] left-[-10%] w-[45%] h-[45%] rounded-full bg-secondary/5 blur-[120px]" />
                ...
                <div className="flex items-center gap-8">
                    <div className="relative group">
                        ...
                        <input
                            ...
                            className="bg-slate-950/60 border border-white/5 pl-14 pr-8 py-4 text-[10px] font-black uppercase tracking-widest text-white outline-none focus:border-azure-500/50 transition-all w-72 rounded-2xl shadow-inner placeholder:text-slate-900 italic focus:ring-4 focus:ring-azure-500/5"
                        />
                    </div>
                    <button
            ...
            {loading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 relative z-10">
                    {[1, 2, 3].map(i => (
                        <div key={i} className="h-72 glass-panel !bg-slate-950/40 rounded-[3rem] border-white/5 animate-pulse" />
                    ))}
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 relative z-10 animate-in fade-in duration-700">
                    {filteredNotes.map((note) => (
                        <motion.div
                            ...
                            key={note.id}
                            className="glass-panel !bg-slate-950/40 p-10 rounded-[3rem] border border-white/5 hover:border-white/10 transition-all group relative flex flex-col h-80 overflow-hidden shadow-2xl backdrop-blur-3xl"
                        >
                            ...
                            <div className="flex justify-between items-start mb-6 relative z-10">
                                ...
                                <div className="flex space-x-3 opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0">
                                    <button onClick={() => { setEditingNote(note); setIsFormOpen(true); }} className="p-3 bg-white/5 border border-white/10 rounded-xl text-slate-400 hover:text-white transition-all shadow-2xl">
                                        <Save size={16} />
                                    </button>
                                    <button onClick={() => handleDelete(note.id)} className="p-3 bg-white/5 border border-white/10 rounded-xl text-slate-400 hover:text-red-500 transition-all shadow-2xl">
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between pt-6 border-t border-white/5 relative z-10">
                                <div className="flex items-center gap-3">
                                    <div className="w-2 h-2 rounded-full bg-azure-500 shadow-[0_0_8px_rgba(45,212,191,0.5)]" />
                                ...
                                {note.pinned && (
                                    <div className="p-2 bg-secondary/10 border border-secondary/20 rounded-lg text-secondary animate-pulse shadow-2xl shadow-secondary/20">
                                        <Pin size={12} />
                                    </div>
                                )}
                            </div>
                    ...
                    {filteredNotes.length === 0 && (
                        <div className="col-span-full py-40 text-center border border-dashed border-white/5 rounded-[4rem] bg-white/[0.01]">
                            <div className="w-24 h-24 bg-slate-900 text-slate-800 rounded-[2rem] flex items-center justify-center mx-auto mb-8 border border-white/5 shadow-inner">
                                <Search size={40} />
                            </div>
                                ...
                                <div className="flex items-center space-x-6">
                                    ...
                                    <div>
                                        <h2 className="text-3xl font-black uppercase tracking-tighter text-white italic">
                                            {editingNote ? 'EDITAR <span className="text-secondary">NODO</span>' : 'REDACTAR <span className="text-secondary">ACTIVO</span>'}
                                        </h2>
    ...
    return (
        <div className="space-y-10">
            <div className="space-y-4">
                <label className="text-[10px] font-black uppercase text-slate-500 tracking-[0.4em] ml-2 italic">Identificador del Nodo</label>
                <input
                    type="text"
                    value={formData.title}
                    onChange={(e) => setFormData({ ...formData, title: e.target.value.toUpperCase() })}
                    placeholder="TÍTULO ESTRATÉGICO..."
                    className="w-full bg-slate-950 border border-white/5 p-6 rounded-[2rem] text-[15px] font-black uppercase tracking-widest text-white shadow-inner focus:border-secondary transition-all outline-none italic placeholder:text-slate-900"
                />
            </div>
            ...
            <div className="space-y-4">
                ...
                <textarea
                    ...
                    className="w-full bg-slate-950 border border-white/5 p-8 rounded-[2.5rem] text-[12px] font-black text-white shadow-inner focus:border-secondary transition-all h-64 outline-none resize-none uppercase tracking-widest leading-relaxed placeholder:text-slate-900 italic custom-scrollbar"
                />
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between pt-10 border-t border-white/5 gap-8">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex space-x-3 bg-slate-950 p-3 rounded-2xl border border-white/5 shadow-inner">
                        {colors.map(c => (
                            <button
                                key={c}
                                type="button"
                                onClick={() => setFormData({ ...formData, color: c })}
                                className={`w-8 h-8 rounded-lg transition-all ${formData.color === c ? 'ring-4 ring-white/10 scale-110 shadow-2xl' : 'opacity-40 hover:opacity-100 hover:scale-105'}`}
                ...
                <button
                    ...
                    className={`flex items-center gap-4 px-6 py-3 rounded-xl border transition-all text-[10px] font-black uppercase tracking-[0.4em] italic shadow-2xl ${formData.pinned ? 'bg-secondary/10 border-secondary/30 text-secondary' : 'bg-slate-950 border-white/5 text-slate-600 hover:text-white'}`}
                >
                    {formData.pinned ? <Pin size={16} className="animate-pulse" /> : <PinOff size={16} />}
                    <span>{formData.pinned ? 'ANCLADO_CRÍTICO' : 'FIJAR_NODO'}</span>
                </button>
            </div>
