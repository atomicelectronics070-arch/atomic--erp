    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setMessages(prev => [...prev, { role: "model", content: "ERROR DE PROTOCOLO: El sistema neuronal no respondi贸. Reintentar conexi贸n." }])
            }
        } catch (error) {
            setMessages(prev => [...prev, { role: "model", content: "ERROR DE RED: Sincronizaci贸n fallida con el n煤cleo de IA Central." }])
        } finally {
            setIsLoading(false)
        }
    }
            ...
            <div className="p-8 bg-slate-950/60 border-t border-white/5 shrink-0 relative z-20 backdrop-blur-3xl">
                <div className="flex space-x-6 relative items-end">
                    <textarea
                        ...
                        placeholder="INICIAR TRANSMISI脫N..."
                ...
                <p className="mt-6 text-center text-[9px] font-black text-slate-800 uppercase tracking-[0.4em] italic leading-none">
                    Gesti髇 de Sincronizaci贸n IA v2.5.0 - N煤cleo <span className="text-secondary/40">Atomic Solutions</span>
                </p>
            </div>
        </div>
    )
