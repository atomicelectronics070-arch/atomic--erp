...
import { Trophy, Send, Plus, History, DollarSign, Target, Award, Loader2, Zap } from "lucide-react"
...
export default function PublicationRankingPanel({ isAdmin }: { isAdmin: boolean }) {
    ...
    const fetchData = async () => {
        try {
            const [rankRes, goalRes] = await Promise.all([
                fetch("/api/publication-ranking"),
                fetch("/api/publication-goal")
            ])
    ...
    const handleAddNumbers = async (userId: string) => {
        ...
        setUpdating(userId)
        try {
            const res = await fetch("/api/publication-ranking", {
                method: "POST",
    ...
    const handleSetGoal = async () => {
        ...
        try {
            const res = await fetch("/api/publication-goal", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    return (
        <div className="space-y-8 w-full">
            {/* WEEKLY GOAL HEADER */}
            <div className="w-full bg-[#0F172A] border-b-4 border-[#3B82F6] p-10 relative overflow-hidden shadow-2xl">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                    <div className="space-y-4 text-center md:text-left">
                        <div className="inline-flex items-center gap-3 px-4 py-1 bg-[#3B82F6]/20 border border-[#3B82F6]/30 rounded-full">
                            <Zap size={12} className="text-[#3B82F6] animate-pulse" />
                            <span className="text-[10px] font-black text-[#3B82F6] uppercase tracking-[0.3em] italic">META DE PUBLICACIONES SEMANAL</span>
                        </div>
                        <h2 className="text-4xl md:text-5xl font-black text-white uppercase italic tracking-tighter leading-[0.9]">
                            {goal && goal.amount > 0 ? (
                                <>PREMIO: <span className="text-sky-400 font-black">${goal.amount}</span> <span className="text-white/20 ml-2">BONO</span></>
                            ) : (
                                "SISTEMA DE PUBLICACIONES"
                            )}
                        </h2>
                        <p className="text-sm font-medium text-slate-400 uppercase italic tracking-widest max-w-xl">
                            {goal?.description || "CADA PUBLICACIÓN CUENTA. LIDERA LA DIFUSIÓN Y GANA EL RECONOCIMIENTO ATÓMICO."}
                        </p>
                    </div>
                        ...
                        <div className="bg-slate-900/50 backdrop-blur-xl border-slate-700/50/5 backdrop-blur-md px-8 py-6 border border-white/10 text-center">
                            ...
                            <div className="flex items-center gap-4">
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-[10px] font-bold text-[#3B82F6] uppercase tracking-widest">07:00 AM</span>
                                </div>
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-[10px] font-bold text-sky-400 uppercase tracking-widest">07:00 PM</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {/* ADMIN GOAL SETTER */}
            {isAdmin && (
                <CyberCard className="!p-8 border-sky-500/20 bg-sky-50/5">
                    <div className="flex items-center gap-4 mb-6">
                        <Target className="text-sky-500" size={24} />
                        <h3 className="text-sm font-black text-navy uppercase italic tracking-widest">META DE PUBLICACIONES</h3>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="md:col-span-1">
                            <input 
                                ...
                                className="w-full bg-slate-900/50 backdrop-blur-xl border-slate-700/50 border border-slate-200 p-4 text-sm font-black text-navy outline-none focus:border-sky-500 italic"
                            />
                        </div>
                        <div className="md:col-span-1">
                            <input 
                                type="text" 
                                placeholder="DETALLE DE META"
                        ...
                        <button 
                            ...
                            className="bg-sky-500 text-white font-black uppercase text-[10px] tracking-widest py-4 hover:bg-sky-600 transition-all italic"
                        >
                            ACTUALIZAR META
                        </button>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-xl font-black text-navy uppercase italic tracking-tighter">RANKING DE PUBLICACIONES REALIZADAS</h2>
                        </div>
                    ...
                    {topAdvisor && (
                        <motion.div 
                            ...
                            className="bg-gradient-to-br from-sky-500/10 to-transparent border-2 border-sky-500/20 p-8 relative overflow-hidden group"
                        >
                            <div className="absolute top-0 right-0 p-4">
                                <Send size={60} className="text-sky-500 opacity-10 group-hover:scale-110 transition-transform" />
                            </div>
                            <div className="flex items-center gap-6 relative z-10">
                                <div className="w-20 h-20 rounded-full border-4 border-sky-500/30 p-1">
                                    <div className="w-full h-full rounded-full bg-sky-500/10 flex items-center justify-center font-black text-2xl text-sky-600 overflow-hidden">
                                        {topAdvisor.user.profilePicture ? (
                                            <img src={topAdvisor.user.profilePicture} alt="" className="w-full h-full object-cover" />
                                        ) : (
                                            topAdvisor.user.name?.[0] || "U"
                                        )}
                                    </div>
                                </div>
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className="text-[10px] font-black text-sky-600 uppercase tracking-widest italic">EDITOR DE LA SEMANA</span>
                                    </div>
                                    ...
                                    <p className="text-xs font-bold text-navy/40 uppercase italic">
                                        LIDERANDO CON <span className="text-[#3B82F6]">{topAdvisor.currentWeekCount} POSTS</span>
                                    </p>
                                </div>
                            </div>
                        </motion.div>
                                        ...
                                        <div className="flex-1">
                                            <div className="flex justify-between items-end mb-1">
                                                <p className="text-[11px] font-black text-navy uppercase italic">
                                                    {index === 0 && <Award size={12} className="inline mr-1 text-sky-500 mb-0.5" />}
                                                    {entry.user.name} {entry.user.lastName}
                                                </p>
                                                <p className="text-xs font-black text-[#3B82F6] italic">{entry.currentWeekCount} <span className="text-[8px] opacity-40 uppercase tracking-tighter">POSTS</span></p>
                                            </div>
                                            <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden border border-slate-200/50">
                                                <motion.div 
                                                    initial={{ width: 0 }}
                                                    animate={{ width: `${Math.min(100, (entry.currentWeekCount / (topAdvisor?.currentWeekCount || 1)) * 100)}%` }}
                                                    className={`h-full ${index === 0 ? 'bg-sky-500' : 'bg-[#3B82F6]'} relative`}
                                                >
                                    ...
                                    {isAdmin && (
                                        <div className="absolute right-0 top-0 opacity-0 group-hover:opacity-100 transition-all flex items-center gap-2 bg-slate-900/50 backdrop-blur-xl border-slate-700/50/80 backdrop-blur-sm p-1 border border-slate-100 z-20">
                                            ...
                                            <button 
                                                ...
                                                className="bg-[#3B82F6] text-white p-1 hover:bg-navy transition-all"
                                            >
                ...
                <div className="space-y-6">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-black text-navy uppercase italic tracking-tighter">HISTORIAL GLOBAL</h2>
                    </div>

                    <CyberCard className="!p-6 space-y-4 bg-slate-50/50">
                        {ranking.map((entry: any) => (
                            <div key={`hist-pub-${entry.id}`} className="flex justify-between items-center border-b border-slate-200/50 pb-3 last:border-0 last:pb-0">
                                <div className="flex items-center gap-3">
                    ...
                    <div className="p-6 border border-dashed border-[#3B82F6]/20 bg-[#3B82F6]/3 text-center rounded-sm">
                        <Send size={24} className="mx-auto mb-3 text-[#3B82F6] opacity-30" />
                        <p className="text-[9px] font-black text-[#3B82F6] uppercase tracking-[0.2em] italic mb-1">TOTAL PUBLICADO</p>
